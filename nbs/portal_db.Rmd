...
institution_grant_df <- min_grant_df %>% 
  ...
  mutate(fullName = case_when(
    ...
    name == "IUPUI" ~ "Indiana University – Purdue University Indianapolis",
...
min_pub_df <- full_pub_df %>% 
  ...
  select(id, grantNumber, title = publicationTitle, journal,
         publicationYear, doi, pubMedUrl = pubMedLink,
         keywords,
         author = authors,
...
# (one-to-many relationship from pub -> links — I think)
...
publication_df <- min_pub_df %>%
  select(id, title, journal, publicationYear, pubMedUrl, doi, keywords) %>% 
  distinct()
...
# manually create the view in Synapse and download (NOTE: need to add placeholder
# columns for addtiional annotations as well), then update the view
pub_synview <- update_view("syn21650536", publication_df %>% mutate(pubMedLink = pubMedUrl))
