...
parser.add_argument('annotation_table_id', type=str,
                    help='Synapse Table id for the annotations to be changed')
parser.add_argument('cv_table_id', type=str,
                    help='Synapse Table id for controlled vocabulary to be referenced')
parser.add_argument('column', type=str,
                    help='Name of column in annotation table to reference, i.e. "assay"')
...
for i, row in annotations_view.iterrows():
    ...
    if isinstance(annots, str):
        annots = [annots]
    new_annots = [cv_view.loc[cv_view['existing'] == x, 'value'].values[0]
                  if x in cv_view['existing'].values else x
                  for x in annots]
