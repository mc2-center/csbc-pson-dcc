...
merged_grant_syntable <- merged_formatted_grant_df %>% 
  # mutate(id = grantName) %>% 
  mutate_at(grant_list_cols, ~ csv_str_to_json(.)) %>%
...
proj_list_cols <- c("grantName", "grantType", "theme", "consortium")
...
merged_formatted_dataset_df <- merged_dataset_df %>%
  ...
  ungroup() %>%
  # rowwise() %>% 
  mutate_at(dataset_list_cols, ~ csv_str_to_json(.)) %>%
  # ungroup() %>% 
  distinct()
...
merged_formatted_tool_df <- merged_tool_df %>%
  ...
  group_by(toolId, toolName, homepageUrl, toolType, role) %>%
  summarize(
