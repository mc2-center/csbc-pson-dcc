...
def get_sim_pubs_from_ids(pmids, titles, csbc_pson_pmids, api_key=None):
    # For each PubMed ID input (not link) retrieves the list of PubMed's recommended 'Similar Articles', and intersects it with the set of CSBC-PSON publications.
    # Returns a pandas series of the titles and links to these similar articles in semicolon separated strings in the same order as the PubMed ID series
    ...
    # Find titles from our titles column, and join them with semicolons
    sim_titles = sim_articles.apply(lambda ids: ";".join([id_title_map[pmid] for pmid in ids]))

    # Construct links from the similar PubMed IDs and join them with semicolons
...
def add_sim_pubs_to_df(df, pmid_col="PubMed", input_title_col="Title", output_link_col="sim_links", output_title_col="sim_titles",
    ...
    # Find PubMed IDs in the input column
    pmids = df[pmid_col].apply(lambda x: [field for field in x.split("/") if field][-1].split("?term=")[-1])

    # Unique PubMed IDs, used to intersect with the query results (may be redundant, since usually pmids will already be unique)
    csbc_pson_pmids = set(pmids)

    # Add the Pandas series generated by get_sim_pubs_from_ids as a column to the input dataframe either in place or on a copy of the dataframe
    if in_place:
